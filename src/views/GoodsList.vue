<template>
    <div>
      ...
      <div class="accessory-result-page accessory-page">
        <div class="container">
          <div class="searchDimContainer">
            <input type="text" placeholder="请输入商品名称" @input="getSeachValue" @keyup.enter="toSearch">
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getSeachValue(event) {
              this.isSearchVal = event.target.value
            },
            toSearch() {
              this.busy = true
              axios.post("/goods/searchDim",{
                searchVal:this.isSearchVal
              }).then((response) => {
                  let res = response.data
                  if(res.status == "0") {
                    if(res.result.count) {
                      this.$Message({message: 'search goods success.', type: 'success', duration: 1500})
                      this.goodsList = res.result.list
                    }else{
                      this.$Message({message: 'goods is null.', type: 'success', duration: 1500})
                    }
                  }else{
                    this.$Message({message: 'goods is null.', type: 'error', duration: 1500})
                    this.goodsList = []
                  }
                })
            }

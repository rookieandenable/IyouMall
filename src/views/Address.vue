<template>
    <div>
                ...
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    ...
                    <div class="addr-opration addr-edit">
                      <a href="javascript:;" class="addr-edit-btn" @click="addAddressConfirm(item)">
                  ...
                  <li class="addr-new" @click="toAddAddress()">
      ...
      <!-- 修改地址 -->
      <modal :addrShow="showAdd" @close="closeAddModal">
        <input type="text" slot="addrName" placeholder="请输入收件人姓名" :value="this.userName" @input="onChangeName">
        <input type="text" slot="addrInfo" placeholder="请输入详细地址" :value="this.streetName" @input="onChangeInfo">
        <input type="text" slot="addrTel" placeholder="请输入联系电话" :value="this.tel" @input="onChangeTel">
        <div slot="btnGroup">
            <a class="btn btn--m" href="javascript:;" @click="modifyAddress">确认</a>
            <a class="btn btn--m btn--red" href="javascript:;" @click="showAdd=false">取消</a>
        </div>
      </modal>
      <!-- 添加地址 -->
      <modal :toAdd="toAddShow" @close="closeAddModalAddr">
        <input type="text" slot="addrNameAdd" placeholder="请输入收件人姓名" @input="onAddName">
        <input type="text" slot="addrInfoAdd" placeholder="请输入详细地址" @input="onAddInfo">
        <input type="text" slot="addrTelAdd" placeholder="请输入联系电话" @input="onAddTel">
        <div slot="btnGroup">
            <a class="btn btn--m" href="javascript:;" @click="addAddress">确认</a>
            <a class="btn btn--m btn--red" href="javascript:;" @click="toAddShow=false">取消</a>
        </div>
      </modal>
      ...
      methods:{
              ...
              axios.post("/users/setDefault",{
                addressId:addressId
                  ...
                  if(res.status=='0'){
                     ...
                     this.$Message({message: 'set default sucess.', type: 'success', duration: 1500})
                     this.init();
                  }
              })
          },
          ...
          delAddress(){
            axios.post("/users/delAddress",{
              addressId:this.addressId
                ...
                if(res.status=="0"){
                  console.log("del suc");
                  this.$Message({message: 'delete address sucess.', type: 'success', duration: 1500})
                  this.isMdShow = false;
                  this.init();
                }
            })
          ...
          modifyAddress() {
            axios.post("/users/addressEdit",{
              addressId:this.addressId,
              userName:this.inputChangeName?this.inputChangeName:this.userName,
              tel:this.inputChangeTel?this.inputChangeTel:this.tel,
              streetName:this.inputChangeInfo?this.inputChangeInfo:this.streetName
            }).then((response) => {
              let res = response.data
              if(res.status == '0') {
                console.log("modify suc")
                this.$Message({message: 'modify address sucess.', type: 'success', duration: 1500})
                this.showAdd = false
                this.init()
              }
            })
          ...
          closeAddModalAddr() {
            this.toAddShow = false
          },
          addAddressConfirm(item) {
            this.showAdd = true;
          ...
          onChangeName(event) {
            this.inputChangeName = event.target.value
          },
          onChangeTel(event) {
            this.inputChangeTel = event.target.value
          },
          onChangeInfo(event) {
            this.inputChangeInfo = event.target.value
          },
          ...
          addAddress() {
            axios.post("/users/addressAdd",{
              userName:this.inputAddName,
              streetName:this.inputAddInfo,
              tel:this.inputAddTel
            }).then((response) => {
              let res = response.data
              if(res.status == '0') {
                console.log("add suc")
                this.$Message({message: 'add address sucess.', type: 'success', duration: 1500})
                this.toAddShow = false
                this.init()
              }
            })
          ...
          onAddName(event) {
            this.inputAddName = event.target.value
          },
          onAddTel(event) {
            this.inputAddTel = event.target.value
          },
          onAddInfo(event) {
            this.inputAddInfo = event.target.value
          }
      }
  }
</script>
